...
@pytest.mark.unit
def test_allows_valid_tools_unchanged():
    """execute_setup und get_bitwig_state sind gültige Phase-3-Tools — Policy lässt sie durch."""
    state = {
    ...
    response = AIMessage(
        content="",
        tool_calls=[
            {"name": "get_bitwig_state", "args": {}, "id": "c1", "type": "tool_call"},
    ...
    assert [tc["name"] for tc in out.tool_calls] == ["get_bitwig_state", "execute_setup"]
...
def test_removes_hallucinated_legacy_tools():
    ...
    assert "get_bitwig_state" in names
...
def test_removes_build_song_dead_tool():
    ...
    response = AIMessage(
        content="",
        tool_calls=[
            {"name": "control_bitwig", "args": {"action": "play"}, "id": "c1", "type": "tool_call"},
...
@pytest.mark.unit
def test_removes_phase3_removed_tools():
    """Phase-3-entfernte Tools (check_bitwig_connection, play_notes, etc.) werden erkannt."""
    state = {"messages": [HumanMessage(content="spiele einen Beat")]}
    response = AIMessage(
        content="",
        tool_calls=[
            {"name": "launchpad", "args": {"action": "play", "note_data": []}, "id": "c1", "type": "tool_call"},
            {"name": "play_notes", "args": {"notes": [{"note": 36}]}, "id": "c2", "type": "tool_call"},
            {"name": "check_bitwig_connection", "args": {}, "id": "c3", "type": "tool_call"},
        ],
    )
    out, meta = enforce_policy_on_response(state, response)
    assert meta["action"] == "rewrite"
    assert "play_notes" in meta["violations"]
    assert "check_bitwig_connection" in meta["violations"]
    names = [tc["name"] for tc in out.tool_calls]
    assert names == ["launchpad"]
