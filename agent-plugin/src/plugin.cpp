// Agent UI CLAP Plugin
// Embeds a Prompt/Response UI in Bitwig's device panel.
// Communicates with the Python agent via OSC (UDP 127.0.0.1:9003).
//
// Build:  see Makefile in parent directory
// Author: bitwig-agent project
...
static constexpr int AGENT_OSC_PORT       = 9003;  // send prompt to
static constexpr int AGENT_RESPONSE_PORT  = 9004;  // receive responses on
static constexpr clap_id TIMER_ID         = 1;
static constexpr uint32_t TIMER_MS        = 50;    // 20 fps
...
struct AgentPlugin {
    ...
    // called from UI/timer thread
    void on_send(const std::string& prompt) {
        if (sender) sender->send_string("/agent/ui/prompt", prompt.c_str());
        // visual feedback: show "Sending…" in response area
        ui.set_response("Sending…");
    }

    // called from timer callback – apply pending response
    void tick() {
...
static bool plugin_init(const clap_plugin_t* p) {
    ...
    // OSC sender (prompt → agent on port 9003)
    ap->sender = std::make_unique<OscSender>("127.0.0.1", AGENT_OSC_PORT);

    // OSC receiver (response ← agent on port 9004)
...
static bool gui_is_api_supported(const clap_plugin_t*, const char* api, bool floating) {
    return !floating && strcmp(api, CLAP_WINDOW_API_X11) == 0;
}
...
static bool gui_create(const clap_plugin_t* p, const char* api, bool floating) {
    if (floating || strcmp(api, CLAP_WINDOW_API_X11) != 0) return false;
    auto* ap = get(p);
    if (ap->gui_created) return true;

    ap->display = XOpenDisplay(nullptr);
    if (!ap->display) return false;
...
static void gui_destroy(const clap_plugin_t* p) {
    ...
    ap->ui.destroy();
    if (ap->display) { XCloseDisplay(ap->display); ap->display = nullptr; }
...
static bool gui_set_parent(const clap_plugin_t* p, const clap_window_t* window) {
    auto* ap = get(p);
    if (!ap->gui_created || !ap->display) return false;

    Window parent = static_cast<Window>(window->x11);

    bool ok = ap->ui.create(ap->display, parent, 0, 0, 0, 0);
    if (!ok) return false;
...
static bool gui_show(const clap_plugin_t* p) {
    auto* ap = get(p);
    if (ap->ui.win) XMapWindow(ap->display, ap->ui.win);
    return true;
}

static bool gui_hide(const clap_plugin_t* p) {
    auto* ap = get(p);
    if (ap->ui.win) XUnmapWindow(ap->display, ap->ui.win);
    return true;
}
...
static const clap_plugin_descriptor_t s_desc = {
    ...
    .version      = PLUGIN_VER,
    .description  = DESC,
    .features     = (const char*[]){ CLAP_PLUGIN_FEATURE_UTILITY, nullptr },
};
