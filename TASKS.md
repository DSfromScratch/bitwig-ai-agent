...
(`tools/bitwig/`, `tools/music/`, `tools/knowledge/`, `tools/meta/`) ist ✅ umgesetzt;
zusätzlich zwei risikoarme OOP-Konsolidierungen (OSC-Client + BaseRepository).
...
| **Phase A — Verzeichnis-Split:** 23 flache Tool-Module per `git mv` nach `bitwig/`+`music/`+`knowledge/`+`meta/`, ~280 Import-Pfade angepasst (`registry.py`/`mcp_bridge.py`/`__init__.py` bleiben top-level), öffentliche API stabil | mehrere | L | ✅ |
| **Phase B — OSC-Client-Konsolidierung:** `configure_dgram_socket()`-Helfer in `osc/client.py`, 8 duplizierte Socket-Setups in `track_state.py`/`song_tools.py`/`suggest_tools.py` zentralisiert (verhaltenserhaltend), 6 Unit-Tests | `osc/client.py` + Tools | M | ✅ |
| **Phase C — BaseRepository:** `_query_one()`/`_query_many()` zentralisieren `is_available`-Guard + `session()`-Kontext + Fehler-Logging; 6 Read-Methoden in 5 Repos migriert (Writes/Vektor-Queries unberührt), 6 Unit-Tests | `src/knowledge/repositories.py` + `tests/test_base_repository.py` (neu) | M | ✅ |
...
| # | Aufgabe | Datei | Aufwand |
|---|---------|-------|---------|
| 1 | ✅ **Return-Track + Send-Routing (C.1)** — `add_track` (track_type=`return`→`createEffectTrack`) war vorhanden; fehlte der `set_send`-Executor in Java. Neu: `execSetSend()` (Track→`sendBank().getItemAt(send_index)`, Pegel 0..1, Existenz-/Range-Guards), Sends in `init()` `markInterested`, Precondition + Dispatcher-Case. Python (`SetSendStep`, Builder, Bridge-`_SETUP_TYPES`) war bereits vorhanden → Round-Trip jetzt vollständig | `BitwigStepPluginExtension.java` | M |
| 2 | ✅ **Track-Gruppen (C.2)** — `add_track` (StepPlugin, aktiver Agent-Pfad) unterstützt jetzt `track_type="group"` via Bitwig-Action `create_group_track` (kein direktes `createGroupTrack` in der API; Pattern aus `BitwigAgentBridgeExtension` übernommen, mit Fehler-Guard). Python: `AddTrackStep`-Literal + `project_template.TrackType` um `group` erweitert, neues `GroupTrackPlugin` (Group-Bus + optionale FX wie Glue-Compressor) in `TRACK_PLUGINS`, Prompt/State-Doku. Tests: +1 Java (32) + 4 Python (`test_step_models.py`, `test_template_workflow.py`) | `BitwigStepPluginExtension.java` | S |
| 3 | ✅ **Drum-Machine Multi-Pad-Setup (C.3)** — neuer Step `setup_drum_machine` (`{track_index, pads:[{pad|note, name, uuid?}]}`): `execSetupDrumMachine()` lädt Drum Machine via UUID, belegt dann Pads sequenziell (gestaffelt, `drumPadBank` folgt cursorDevice) mit Built-in-Devices per UUID; Sample-/VST-Pads ohne UUID-Treffer werden geloggt+übersprungen. Neuer `JsonStepParser.splitObjects()` (tiefen-bewusst). Python: `SetupDrumMachineStep`-Modell + Builder + Bridge-`_SETUP_TYPES` + Prompt/State-Doku. Tests: 6 Java (`JsonStepParserTest`) + 6 Python (`test_step_models.py`) | `BitwigStepPluginExtension.java` | M |
| 4 | ✅ `call_llm()`-Split — bereits erledigt: `call_llm()` ist nur noch ein 37-Zeilen-Orchestrator, delegiert an `_invoke_with_retry()`, `_prepare_messages()`, `_process_reasoning()`, `_recover_tool_calls()`, `_handle_invalid_output()`, `_apply_policy()` (Nudge-Logik). Keine 214-Zeilen-Funktion mehr | `src/agent/core.py` | M |
| 5 | Jazz-Trainingsdaten erweitern (Ride statt HiHat, Offbeat-Snare, Walking Bass) | `~/mlx-training/train.jsonl` | L |
| 6 | ✅ **`_query_neo4j()`-Split** (372 Zeilen) in 8 fokussierte, einzeln testbare Sektions-Funktionen: `_extract_keywords()`, `_section_concepts/_devices/_genres/_workflows/_similar_devices/_artists/_songs/_production_patterns()`. `_query_neo4j()` ist jetzt ein schlanker Orchestrator. Latenter Bug behoben: `import json` von inline (Sektion 2/4) auf Modul-Ebene gezogen — verhinderte `UnboundLocalError` bei reinen Artist-Treffern. 7 mock-basierte Tests `tests/test_query_neo4j_sections.py`, Live gegen Neo4j verifiziert (Verhalten identisch) | `src/agent/tools/knowledge_tool.py` | M |
...
| 8 | ✅ **AudioSample → SoundRecipe** verknüpfen — robuste, projekt-agnostische `SAMPLED_IN`-Verknüpfung (ersetzt hardcoded Projekt-Mappings). Migration `src/knowledge/migrations/link_audio_samples.cypher` (Regel A: Bounce-Stem == `track_name`; Regel B: Stem == eindeutiges `primary_device`), Runner `scripts/link_audio_samples.py`, Ingest-Fix (`_link_samples_robust` in `ingest_audio_samples.py`), Consumer in `knowledge_tool.py` („🎚️ Samples:"). Live: 50→60 Kanten, 32/54 Samples robust verknüpft, idempotent | KB-Migration + Ingest + Tool | M |
| 9 | ✅ **Artist-Node** + `get_artist_context()` Tool — neues Tool `src/agent/tools/artist_tool.py` (registriert, Domain `knowledge`): liefert Stil-Profil, Genre, BPM/Tonart, charakteristische Devices, assoziierte Genres (via `:ASSOCIATED_WITH`), Referenz-Songs (via `:BY`) und klanglich ähnliche Songs (via `:SIMILAR_TO` aus C.10). Teil-Treffer + Vorschlagsliste bei Fehlschlag. 4 mock-basierte Tests | `tools/`, KB-Migration | M |
...
| 12 | ✅ **Device-Dedup** — Case-Duplikat-Nodes („Poly Grid" vs „poly grid") zusammengeführt. Migration `src/knowledge/migrations/dedup_device_nodes.cypher` (übernimmt `builtin_uuid`/`load_cmd` in die kanonische Title-Node bevor die dünne lowercase-Variante gelöscht wird), Runner `scripts/dedup_device_nodes.py` mit Relationen-Sicherheits-Check. Live: 568→441 Devices (−127), `USES_DEVICE` intakt (190), idempotent | KB-Migration | S |
